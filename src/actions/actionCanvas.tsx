...
export const actionZoomToFrame = register({
  name: "zoomToFrame",
  trackEvent: { category: "canvas" },
  perform: (_elements, appState, targetFrame, app) => {
    app.scene.setCurrentFrame(targetFrame);
    return zoomToFit({
      targetElements: [targetFrame],
      appState,
      fitToViewport: true,
    });
  },
...
export const actionZoomToNextFrame = register({
  name: "zoomToNextFrame",
  trackEvent: { category: "canvas" },
  perform: (elements, appState, _, app) => {
    const frames = app.scene.getNonDeletedFrames();
    const currentFrame = app.scene.getCurrentFrame();
    const currentFrameIndex = frames.findIndex((f) => f === currentFrame);
    const nextFrameIndex =
      currentFrameIndex + 1 === frames.length
        ? currentFrameIndex
        : currentFrameIndex + 1;
    const target: readonly ExcalidrawElement[] =
      frames.length === 0
        ? elements
        : currentFrameIndex === -1
        ? [frames[0]]
    ...
    if (frames.length > 0) {
      app.scene.setCurrentFrame(frames[nextFrameIndex]);
    }
    return zoomToFit({
...
export const actionZoomToPreviousFrame = register({
  name: "zoomToPreviousFrame",
  trackEvent: { category: "canvas" },
  perform: (elements, appState, _, app) => {
    const frames = app.scene.getNonDeletedFrames();
    const currentFrame = app.scene.getCurrentFrame();
    const currentFrameIndex = frames.findIndex((f) => f === currentFrame);
    const nextFrameIndex =
      currentFrameIndex === 0 ? currentFrameIndex : currentFrameIndex - 1;
    const target: readonly ExcalidrawElement[] =
      frames.length === 0
        ? elements
        : currentFrameIndex === -1
