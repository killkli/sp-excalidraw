...
class Collab extends PureComponent<Props, CollabState> {
  ...
  stopCollaboration = (keepRemoteState = true) => {
    ...
    if (!keepRemoteState) {
      ...
      if (window.confirm("接下來將會停止與遠端的共享，目前的畫面可能會受到其他頁面影響而改變內容，建議先存檔目前狀態，是否要先存檔呢？")) {
        this.excalidrawAPI.actionManager.executeAction(this.excalidrawAPI.actionManager.actions["saveFileToDisk"])
      }
  ...
  startCollaboration = async (
    ...
    try {
      ...
      console.log("socket data:", socketServerData);

      this.portal.socket = this.portal.open(
        socketIOClient(socketServerData.url, {
          path: "/_exdraw_ws/socket.io",
