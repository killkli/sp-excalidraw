...
export const RoomModal = ({
  ...
  const setRoomNameAction = (name: string) => {
    document.title = `${name} - Excalidraw`;
    setRoomName(name);
  }
  const winTitleName = document.title.split(' - ')[0];
  const [room_name, setRoomName] = useState(winTitleName.startsWith('Excalidraw') ? '' : winTitleName);
  if (activeRoomLink) {
    return (
      <>
        ...
        <QRCode value={activeRoomLink} size={256} />
        <TextField
          value={room_name}
          placeholder="你的標題..."
          label="設定分享標題"
          onChange={setRoomNameAction}
